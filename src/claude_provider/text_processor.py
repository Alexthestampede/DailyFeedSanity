"""
Text processing with Anthropic Claude for RSS Feed Processor

This module provides text summarization and title generation using Anthropic's
Claude API.
"""
...
class ClaudeTextClient(BaseTextProcessor):
    """
    Text processor using Anthropic Claude for summarization and title generation.

    This class implements the same interface as OllamaTextClient, allowing
    for transparent provider switching.
    """

    def __init__(self, api_key: str, model: str = CLAUDE_TEXT_MODEL):
        """
...
Respond with ONLY "yes" or "no" - nothing else."""
            ...
            if response:
                ...
                if is_clickbait:
                    logger.info(f"Clickbait detected by Claude for: {title[:50]}...")
                return is_clickbait
            ...
            logger.error(f"Error in Claude clickbait detection: {e}")
            return False
