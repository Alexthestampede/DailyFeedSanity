...
LM_STUDIO_VISION_MODEL = (
    "qwen/qwen3-vl-4b"  # Model for vision processing (must support image inputs)
)
...
FEED_TYPE_DETECTION_TEMPERATURE = (
    0.1  # Low temperature for consistent feed classification
)
CLICKBAIT_DETECTION_TEMPERATURE = (
    0.1  # Low temperature for consistent clickbait detection
)
LANGUAGE_DETECTION_TEMPERATURE = (
    0.1  # Low temperature for consistent language detection (per-article, deprecated)
)
FEED_LANGUAGE_DETECTION_TEMPERATURE = (
    0.1  # Low temperature for consistent feed language detection
)
AD_DETECTION_TEMPERATURE = (
    0.1  # Low temperature for consistent ad/sponsored content detection
)
...
# Time Filtering
TIME_FILTER_HOURS = (
    24  # Filter news articles to last N hours (can be overridden with --all-entries)
)
...
FEED_TYPE_CACHE_FILE = os.path.join(
    BASE_DIR, ".feed_type_cache.json"
)  # Cache for Ollama feed classification
FEED_TYPE_OVERRIDES_FILE = os.path.join(
    BASE_DIR, "feed_type_overrides.txt"
)  # Manual feed type overrides
FEED_LANGUAGE_CACHE_FILE = os.path.join(
    BASE_DIR, ".feed_language_cache.json"
)  # Cache for feed language detection
FEED_LANGUAGE_OVERRIDE_FILE = os.path.join(
    BASE_DIR, "feed_language_overrides.txt"
)  # Manual feed language overrides
