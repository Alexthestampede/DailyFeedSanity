"""
Text processing with Ollama for RSS Feed Processor
"""
...
class OllamaTextClient:
    ...
    def detect_clickbait(self, title, text):
        ...
        try:
            ...
            if 'yes' in response_lower:
                logger.info(f"Ollama detected clickbait: {title[:50]}...")
                return True
            else:
                return False
    ...
    def generate_summary(self, text, title=None, author=None, language="English", max_length=500):
        ...
        # Check for clickbait using Ollama AI (with fallback on error)
        is_clickbait_ollama = False
        if title:
            try:
                is_clickbait_ollama = self.detect_clickbait(title, text)
            except Exception as e:
                logger.warning(f"Ollama clickbait detection failed, falling back to author-only: {e}")

        # Combine both detection methods
        is_clickbait = is_clickbait_author or is_clickbait_ollama

        # Determine detection source for transparency
        if is_clickbait_author and is_clickbait_ollama:
            clickbait_detected_by = "both"
            logger.info(f"Clickbait detected by both author ({author}) and Ollama")
        elif is_clickbait_author:
            clickbait_detected_by = "author"
            logger.info(f"Clickbait detected by author: {author}")
        elif is_clickbait_ollama:
            clickbait_detected_by = "ollama"
            logger.info(f"Clickbait detected by Ollama for: {title[:50]}...")
        else:
